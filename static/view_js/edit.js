...
function resetTheState() {
  ...
  while (disabledStack.length > 0) disabledStack.pop();
...
function getDataFromDB() {
  ...
  while (dbSlots.length > 0) dbSlots.pop();
  ...
  for (let i = 0; i < dbSlots.length; i++) {
    // Remove the seconds field off start and end time (i.e :00)
    dbSlots[i].startTime = dbSlots[i].startTime.substring(
      0,
      dbSlots[i].startTime.length - 3
    );
    dbSlots[i].endTime = dbSlots[i].endTime.substring(
      0,
      dbSlots[i].endTime.length - 3
    );
  }
...
function initTimeState() {
  ...
  while (existingEventsArray.length > 0) existingEventsArray.pop();
  ...
  for (let i = 0; i < dbExistingSlots.length; i++) {
    ...
    appendToExistingEventTable(
      dbObjToReadable.date,
      dbObjToReadable.dayName,
      dbObjToReadable.startTime,
      dbObjToReadable.endTime
    );
  }
...
function createDisabledInstance(
  arrayToMoveFrom,
  toBeRemovedSlots,
  newSlots,
  tag
...
function editSlot(datesToBeMoved, toBeMovedRow) {
  ...
  endTime = formatTime(
    parseInt(startTime, 10) + parseInt(stateOfEvent.duration, 10)
  );
...
$("#editExistingSlots").on("click", function () {
  ...
  $("#existingEventsTable tbody tr").each(function () {
    if ($(this).hasClass("selectedRow")) toBeMovedRow = $(this);
  });
...
$(document).ready(function () {
  ...
  $("#datepicker")
    ...
    .on("changeDate", function (e) {
      ...
      var hasColumn =
        $("#timeSelector thead th").filter(function () {
      ...
      if (hasColumn === false && e.format() != "") {
        ...
        dragTable(); // Add event handler
      } else if (hasColumn === false && e.format() == "") {
        // First column remove edge case
        removeColumn();
        dragTable(); // Add event handler
      } else if (hasColumn === true && e.format() != "") {
        // Remove columns
        removeColumn();
        dragTable(); // Add event handler
      }
...
function addNewCol(e) {
  ...
  $("#timeSelector tr")
    ...
    .each(function () {
      ...
      var allSlotsTempArray = existingEventsArray.concat(
        stateOfEvent.addedSlots
      );
...
function getEventInFormatFromTableCells(tableRow) {
  ...
  var formatStringObj = {
    displayValue:
      ...
      ", " +
      formattedEventString[1] +
      ", " +
      formattedEventString[2] +
      " - " +
      formattedEventString[3],
    startTime:
      yyyy_mm_dd_format + " " + convertAMPMToMilitary(formattedEventString[2]),
    endTime:
      yyyy_mm_dd_format + " " + convertAMPMToMilitary(formattedEventString[3])
  };
...
function resetCanceledInput() {
  ...
  if (stateOfEvent.duration != findSelectedDuration.val())
    $("#durationSelector")
      .find('option[value="' + stateOfEvent.duration + '"]')
...
$("#timeslotCapInput").on("change", function (e) {
  if (stateOfEvent.dbCapacity < $("#timeslotCapInput").val()) {
    ...
    buildModalForChangeConfirmation(
      "Confirm Change",
      "Saving a time slot capacity less than the current capacity will only apply to slots with enough space to be decremented. Time slots with users booked to that slot that EXCEEDS the new capacity will retain it's original size. "
    );
...
$("#durationSelector").on("change", function (e) {
  buildModalForChangeConfirmation(
    "Confirm Change",
    "Saving a change to the event duration will remove ALL EXISTING EVENT SLOTS and USERS tied to this event."
  );
  ...
  $("#generalAcceptButton").on("click", function () {
    ...
    if (
      parseInt($("#durationSelector option:selected").val(), 10) ===
      parseInt(stateOfEvent.dbDuration, 10)
    ) {
      $(".disabledRow").each(function () {
        $(this).removeClass("disabledRow");
        $(".disabledRow input[type=checkbox]").prop("checked", false);
      ...
      for (let i = 0; i < existingEventsArray.length; i++) {
        var dbObjToReadable = databaseDateFormatToReadable(
          existingEventsArray[i]
        );
        appendToExistingEventTable(
          dbObjToReadable.date,
          dbObjToReadable.dayName,
      ...
      $("#existingEventsTable tbody tr").each(function () {
        ...
        $(this).removeClass("selectedRow");
        tempDisabledExistingEvents.push(
          getEventInFormatFromTableCells($(this))
        );
        $(".disabledRow input[type=checkbox]").prop("checked", true);
        $(".disabledRow input[type=checkbox]").on("click", function (e) {
          e.preventDefault();
...
function changedDuration(timeTablePickerSelectorID) {
  ...
  var selectedDuration = parseInt(
    $("#durationSelector").find(":selected").val(),
    10
  );
...
function buildModalForTimeSave(
  modalHeaderName,
  addArray,
  deleteArray,
  newDuration,
  ...
  if (newDuration == stateOfEvent.dbDuration)
    ...
    durationContainerField.append(
      "<text>Saving this change will remove all users off the event!</text>"
    );
  ...
  if (newCapacity == stateOfEvent.dbCapacity)
    ...
    if (newCapacity < stateOfEvent.dbCapacity) {
      ...
      capacityContainerField.append(
        "<text>Saving this change may remove users off an existing slot!</text>"
      );
    }
...
$("#saveSlots").on("click", function () {
  ...
  //console.log(disabledStack);
  $(
    "#existingEventsTable tr td:nth-last-child( " + eventDeleteIndex + " )"
  ).each(function () {
    if (
      !$(this).parent().hasClass("disabledRow") &&
  ...
  for (
    let i = 0;
    i < disabledStack.length;
    i++ // Get the disabled slots back into the delete array
  )
    deleteObjInfoHolder = deleteObjInfoHolder.concat(
      disabledStack[i].disabledSlots
    );

  if (
    deleteObjInfoHolder.length < 1 &&
    stateOfEvent.addedSlots < 1 &&
    stateOfEvent.capacity == stateOfEvent.dbCapacity
  ) {
  ...
  if (stateOfEvent.addedSlots.length < 1) {
    if (
      getExistingEventsFromDBCachedSlots(deleteObjInfoHolder).length ==
      dbSlots.length
    ) {
      alert(
        "Must have at added at least one time slot or have least one remaining existing time slot"
      );
      return;
    }
  }

  buildModalForTimeSave(
    "Confirm Save",
    stateOfEvent.addedSlots,
    deleteObjInfoHolder,
    stateOfEvent.duration,
    stateOfEvent.capacity
  );
  $("#generalConfirm").modal("toggle");
  ...
  $("#generalAcceptButton").one("click", function () {
    saveTimeChanges(
      stateOfEvent.addedSlots,
      getExistingEventsFromDBCachedSlots(deleteObjInfoHolder),
      stateOfEvent.duration,
      stateOfEvent.capacity
    );
    resetTheState();
...
function arraysNoDuplicate(superset, subset) {
  ...
  if (createSet.size < unionSet.length)
    // Duplicate detected a removed. Therefore there are duplicates
    return false;
  else return true;
}
...
$("#addEventsButton").on("click", function () {
  ...
  if (addEventsCheck === false || $("#Dates").val() == "")
    //addEventsCheck is false when incorrect information is passed. Otherwise it is the slots from adding.
    ...
    if (arraysNoDuplicate(allSlotsTempArray, addEventsCheck) === true) {
      ...
      $("#addEventModal").modal("toggle");
    } else alert("Duplication event slot detected");
  }
});
...
function convertMilitaryTimeToAMPM(timeInMilitaryFormat) {
  ...
  var hours =
    parseInt(timeVal[0], 10) > 12
      ? parseInt(timeVal[0], 10) - 12
      : parseInt(timeVal[0], 10);
  var AM_PM = parseInt(timeVal[0], 10) >= 12 ? "PM" : "AM";
  hours = hours < 10 ? hours : hours;
  var minutes =
    parseInt(timeVal[1], 10) < 10
      ? "0" + parseInt(timeVal[1], 10)
      : parseInt(timeVal[1], 10);
