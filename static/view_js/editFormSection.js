...
const formState = {
  // This is the "state" object that manages the form page of the edit form section. Replace with a class maybe?
  eventName: "",
  eventLocation: "",
  eventDescription: "",
...
$("#fileUpload").on("click", function (e) {
  ...
  buildModalForChangeConfirmation(
    "Confirm Change",
    "Saving this field unchecked (File Upload Field) will cause files currently uploaded to this event to be deleted"
  );
...
$("#anonymousCheck").on("click", function (e) {
  ...
  buildModalForChangeConfirmation(
    "Confirm Change",
    "Saving this field unchecked (Anonymous Field) will make registered users visible to other users upon event registration."
  );
...
function buildModalForFormSave(modalHeaderName) {
  ...
  for (var key of keys) {
    if (typeof eventSaveVals[key] === "boolean") {
      ...
      eventSaveVals[key] === "" ||
      eventSaveVals[key] == eventDbVals[key]
    ) {
...
$("#saveForm").on("click", function () {
  ...
  for (var key of keys) {
    if (eventSaveVals[key] == eventDbVals[key]) continue;
    else {
...
function saveFormChanges() {
  ...
  $.ajax({
    type: "POST",
    url: "src/edit_event_details.php",
    data: {
