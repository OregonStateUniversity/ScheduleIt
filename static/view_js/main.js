...
function createEventBlock(
  ...
  //var newEventIcon = $('<div><img src="static/images/icon.png"></img></div>');
  var newEventIcon = $(
    '<div><img src="static/images/MyEventBoardLogos/MEB_logo-04.png"></img></div>'
  );
  ...
  if (
    checkTimeIfLessThanToday(eventTime, todayTimeStamp) == true &&
    getDate(date) === eventDate
  ) {
...
function buildContainer(events) {
  ...
  for (let i = 0; i < events.length; i++) {
    newEvent = createEventBlock(
      events[i].event_name,
      formatDate(events[i].start_time),
      events[i].ec_first_name,
      events[i].slots_remaining,
...
function isInBetween(currDate, minDate, maxDate) {
  // Check if the date is between two dates. Used for checking if event is in this week.
...
function noEvents() {
  // Add no events label if no reservations exist
  $(".weeksEvent").append(
    "<div><h2> No Upcoming Events for this Week </h2></div>"
  );
}
