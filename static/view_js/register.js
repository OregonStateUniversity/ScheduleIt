...
function checkTimeIfLessThanToday(timeToBeChecked, todaysTime) {
  if (
    !(
      timeToBeChecked.includes("AM") ||
      timeToBeChecked.includes("PM") ||
      todayTimeStamp.includes("PM") ||
      todayTimeStamp.includes("AM")
    )
  ) {
  ...
  var checkTime =
    parseInt(checkedArrayHour[0]) * 60 + parseInt(checkedMinutesArray[0]);
  var todayTime =
    parseInt(todayArrayHour[0]) * 60 + parseInt(todayMinutesArray[0]);

  if (checkTime < todayTime) return true;
  else return false;
}
...
function saveRegister() {
  ...
  if (selectedSlot === false) {
    ...
    checkTimeIfLessThanToday(
      selectedSlot.prev().children().text(),
      getCurrentTime()
    ) == true &&
...
$(document).ready(function () {
  ...
  if (fileOption == false) $(".fileUploadContainer").remove();
});
...
$(function () {
  ...
  if (past_event == true) {
    ...
    $(".calendarField").append(
      '<h2> This Event has Expired <i class="fa fa-frown-o" aria-hidden="true"></i></h2>'
    );
    return;
  }
...
$(document).ready(function () {
  if ($("#datepicker2") != undefined || $("#datepicker2") != null) {
    ...
    if (
      document.getElementById("datepicker2") != null ||
      document.getElementById("datepicker2") != undefined
    )
      document
        .getElementById("datepicker2")
        .addEventListener("click", highlightCalendar);
  }
});
...
function createFields() {
  ...
  for (var timeSlotKey of timeSlotKeys) {
    // store current day times into an array to loop through
    var timeSlot = timeSlotObjects[timeSlotKey];

    tempDateHolder =
      timeSlot.start_time.month +
      "/" +
      timeSlot.start_time.day +
      "/" +
      timeSlot.start_time.year;

    if (selectedDate.localeCompare(tempDateHolder) === 0) {
      times.push(
        twentyFourFormatToMinutes(
  ...
  for (var i = 0; i < times.length; i++) {
    ...
    if (
      parseInt(times[i - 1], 10) + parseInt(selectedDuration, 10) !==
      parseInt(times[i], 10)
    ) {
      addBlankSpace();
      createCells(times[i], spaces[i], isAvailable[i], slotIDs[i], isMySlot[i]);
...
function selectASlot() {
  $("#slotPicker td").click(function () {
    if (
      $(this).parent().hasClass("fullSlot") ||
      $(this).parent().hasClass("mySlot")
    )
      return;

    var check = getColumnSelect();

    if (check === false) $(this).toggleClass("slotSelected");
    else {
...
function totalMinutesToFormat(totalMinutes) {
  ...
  if (minutes === 0) timeString = hours + ":" + "00 " + format;
  else timeString = hours + ":" + minutes + " " + format;
...
function getMonthDayAvailableSpace() {
  ...
  for (var key of objectKeys) {
    ...
    if (!monthDayAvailableSpace[month][day])
      monthDayAvailableSpace[month][day] = 0;
...
function highlightCalendar() {
  var calendarTitle = document.getElementsByClassName("datepicker-switch")[0]
    .textContent;
  ...
  for (var timeSlotKey of Object.keys(timeSlotObjects)) {
    ...
    for (day of calendarDays) {
      ...
      var space =
        monthDayAvailableSpace[monthEnum[calendarMonth]][day.textContent];
